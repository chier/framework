...
public class UpsertPlugin extends BasePlugin {
    ...
    public boolean validate(List<String> warnings) {
        ...
        if ("com.mysql.jdbc.Driver".equalsIgnoreCase(this.getContext().getJdbcConnectionConfiguration().getDriverClass()) == false) {
            warnings.add("itfsw:插件" + this.getClass().getName() + "插件使用前提是数据库为MySQL！");
            return false;
        }
        ...
        if (this.allowMultiQueries) {
            // 提示用户注意信息
            warnings.add("itfsw:插件" + this.getClass().getName() + "插件您开启了allowMultiQueries支持，注意在jdbc url 配置中增加“allowMultiQueries=true”支持（不怎么建议使用该功能，开启多sql提交会增加sql注入的风险，请确保你所有sql都使用MyBatis书写，请不要使用statement进行sql提交）！");
        }
    ...
    private void generateXmlElementWithSelective(Document document, IntrospectedTable introspectedTable) {
        ...
        if (this.allowMultiQueries) {
            ...
            // 查询值
            this.generateExistsClause(introspectedTable, updateEle, insertValuesEle.getElements());
