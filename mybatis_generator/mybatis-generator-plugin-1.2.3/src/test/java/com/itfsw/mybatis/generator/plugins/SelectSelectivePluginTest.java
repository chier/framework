	...
	public void testSelectByExampleSelective() throws Exception {
		...
		tool.generate(new IBeforeCallback() {
			...
			public void reloadProject(SqlSession sqlSession, ClassLoader loader, String packagz) throws Exception {
				...
				// java
				// 动态参数不能有两个会冲突，最后一个封装成Array!!!必须使用反射创建指定类型数组，不然调用invoke对了可变参数会检查类型！
	...
	public void testSelectByPrimaryKeySelective() throws Exception {
		...
		tool.generate(new IBeforeCallback() {
			...
			public void reloadProject(SqlSession sqlSession, ClassLoader loader, String packagz) throws Exception {
				...
				// sql
				String sql = SqlHelper.getFormatMapperSql(tbBlobsMapper.getObject(), "selectByPrimaryKeySelective", 4L,
						columns);
