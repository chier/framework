...
public class UpsertPluginTest {
	...
	public void testWarnings()
			throws IOException, XMLParserException, InvalidConfigurationException, InterruptedException, SQLException {
		// 1. 没有使用mysql数据库
		MyBatisGeneratorTool tool = MyBatisGeneratorTool
				.create("scripts/UpsertPlugin/mybatis-generator-with-error-driver.xml");
		tool.generate();
		Assert.assertEquals(tool.getWarnings().get(1),
				"itfsw:插件com.itfsw.mybatis.generator.plugins.UpsertPlugin插件使用前提是数据库为MySQL！");

		// 2. 普通提示
		tool = MyBatisGeneratorTool.create("scripts/UpsertPlugin/mybatis-generator.xml");
		tool.generate();
		Assert.assertEquals(tool.getWarnings().get(0),
				"itfsw:插件com.itfsw.mybatis.generator.plugins.UpsertPlugin插件您开启了allowMultiQueries支持，注意在jdbc url 配置中增加“allowMultiQueries=true”支持（不怎么建议使用该功能，开启多sql提交会增加sql注入的风险，请确保你所有sql都使用MyBatis书写，请不要使用statement进行sql提交）！");
	}
	...
	public void testUpsert() throws Exception {
		...
		tool.generate(new IBeforeCallback() {
			...
			public void run() throws Exception {
				DBHelper.resetDB("scripts/UpsertPlugin/init.sql");
			}
			...
			public void reloadProject(SqlSession sqlSession, ClassLoader loader, String packagz) throws Exception {
				...
				Assert.assertEquals(sql,
						"insert into tb (id, field1, field2) values (10, 'ts1', 5) on duplicate key update id = 10, field1 = 'ts1', field2 = 5");
	...
	public void testUpsertWithBLOBs() throws Exception {
		...
		tool.generate(new IBeforeCallback() {
			...
			public void reloadProject(SqlSession sqlSession, ClassLoader loader, String packagz) throws Exception {
				...
				// sql
				String sql = SqlHelper.getFormatMapperSql(TbBlobsMapper.getObject(), "upsertWithBLOBs",
				...
				// 1. 单个blob
				ObjectUtil TbSingleBlobMapper = new ObjectUtil(
						sqlSession.getMapper(loader.loadClass(packagz + ".TbSingleBlobMapper")));
				...
				// sql
				sql = SqlHelper.getFormatMapperSql(TbSingleBlobMapper.getObject(), "upsertWithBLOBs",
						TbSingleBlob.getObject());
				Assert.assertEquals(sql,
	...
	public void testUpsertSelective() throws Exception {
		...
		tool.generate(new IBeforeCallback() {
			...
			public void reloadProject(SqlSession sqlSession, ClassLoader loader, String packagz) throws Exception {
				...
				Assert.assertEquals(sql,
						"insert into tb ( id, field1 ) values ( 20, 'ts1' ) on duplicate key update id = 20, field1 = 'ts1'");
				...
				// sql
				sql = SqlHelper.getFormatMapperSql(TbBlobsMapper.getObject(), "upsertSelective",
						TbBlobsWithBLOBs.getObject());
				Assert.assertEquals(sql,
						"insert into tb_blobs ( id, field1, field2 ) values ( 500, 'ts1', 'ts2' ) on duplicate key update id = 500, field1 = 'ts1', field2 = 'ts2'");
	...
	public void testUpsertByExample() throws Exception {
		...
		tool.generate(new IBeforeCallback() {
			...
			public void reloadProject(SqlSession sqlSession, ClassLoader loader, String packagz) throws Exception {
				...
				// sql
				String sql = SqlHelper.getFormatMapperSql(tbMapper.getObject(), "upsertByExample", tb.getObject(),
						TbExample.getObject());
				Assert.assertEquals(sql,
						"update tb set id = 50, field1 = 'ts1', field2 = 5 WHERE ( id = '50' ) ; insert into tb (id, field1, field2) select 50, 'ts1', 5 from dual where not exists ( select 1 from tb WHERE ( id = '50' ) )");
	...
	public void testUpsertByExampleWithBLOBs() throws Exception {
		...
		tool.generate(new IBeforeCallback() {
			...
			public void reloadProject(SqlSession sqlSession, ClassLoader loader, String packagz) throws Exception {
				...
				// sql
				String sql = SqlHelper.getFormatMapperSql(TbBlobsMapper.getObject(), "upsertByExampleWithBLOBs",
						TbBlobsWithBLOBs.getObject(), TbBlobsExample.getObject());
				Assert.assertEquals(sql,
						"update tb_blobs set id = 60, field1 = 'ts1', field2 = 'ts2', field3 = 'null' WHERE ( id = '60' ) ; insert into tb_blobs (id, field1, field2, field3) select 60, 'ts1', 'ts2', 'null' from dual where not exists ( select 1 from tb_blobs WHERE ( id = '60' ) )");
				TbBlobsMapper.invoke("upsertByExampleWithBLOBs", TbBlobsWithBLOBs.getObject(),
				...
				// sql
				sql = SqlHelper.getFormatMapperSql(TbSingleBlobMapper.getObject(), "upsertByExampleWithBLOBs",
						TbSingleBlob.getObject(), TbSingleBlobExample.getObject());
				Assert.assertEquals(sql,
						"update tb_single_blob set id = 70, field2 = 3, field1 = 'ts1' WHERE ( id = '70' ) ; insert into tb_single_blob (id, field2, field1) select 70, 3, 'ts1' from dual where not exists ( select 1 from tb_single_blob WHERE ( id = '70' ) )");
				TbSingleBlobMapper.invoke("upsertByExampleWithBLOBs", TbSingleBlob.getObject(),
						TbSingleBlobExample.getObject());
	...
	public void testUpsertByExampleSelective() throws Exception {
		...
		tool.generate(new IBeforeCallback() {
			...
			public void reloadProject(SqlSession sqlSession, ClassLoader loader, String packagz) throws Exception {
				...
				Assert.assertEquals(sql,
						"update tb set id = 100, field1 = 'ts1' WHERE ( id = '100' ) ; insert into tb ( id, field1 ) select 100, 'ts1' from dual where not exists ( select 1 from tb WHERE ( id = '100' ) )");
				...
				// sql
				sql = SqlHelper.getFormatMapperSql(TbBlobsMapper.getObject(), "upsertByExampleSelective",
						TbBlobsWithBLOBs.getObject(), TbBlobsExample.getObject());
				Assert.assertEquals(sql,
						"update tb_blobs set id = 200, field1 = 'ts1', field2 = 'ts2' WHERE ( id = '60' ) ; insert into tb_blobs ( id, field1, field2 ) select 200, 'ts1', 'ts2' from dual where not exists ( select 1 from tb_blobs WHERE ( id = '60' ) )");
				TbBlobsMapper.invoke("upsertByExampleSelective", TbBlobsWithBLOBs.getObject(),
						TbBlobsExample.getObject());
			}
		});
	}
		...
		tool.generate(new IBeforeCallback() {
			...
			public void reloadProject(SqlSession sqlSession, ClassLoader loader, String packagz) throws Exception {
				...
				ObjectUtil tbMapper = new ObjectUtil(
						sqlSession.getMapper(loader.loadClass(packagz + ".TbWithIncIdMapper")));
				...
				String sql = SqlHelper.getFormatMapperSql(tbMapper.getObject(), "upsert", tb.getObject());
				Assert.assertEquals(sql,
						"insert into tb_with_inc_id ( field1, field2 ) values ( 'ts1', 5 ) on duplicate key update field1 = 'ts1', field2 = 5");
				tbMapper.invoke("upsert", tb.getObject());
				// 获取ID
				...
				Assert.assertEquals(sql,
						"insert into tb_blobs_with_inc_id ( field1, field2, field3 ) values ( 'ts1', 'ts2', 'null' ) on duplicate key update field1 = 'ts1', field2 = 'ts2', field3 = 'null'");
				...
				Assert.assertEquals(sql,
						"insert into tb_blobs_with_inc_id ( field1 ) values ( 'ts4' ) on duplicate key update field1 = 'ts4'");
				tbMapper.invoke("upsert", tb.getObject());
				// 获取ID
				id = (Long) tb.get("id");
				...
				// sql
				sql = SqlHelper.getFormatMapperSql(tbMapper.getObject(), "upsertByExample", tb.getObject(),
						tbExample.getObject());
				Assert.assertEquals(sql,
						"update tb_with_inc_id set field1 = 'ts6', field2 = 5 WHERE ( field1 = 'ts6' ) ; insert into tb_with_inc_id ( field1, field2 ) select 'ts6', 5 from dual where not exists ( select 1 from tb_with_inc_id WHERE ( field1 = 'ts6' ) )");
				tbMapper.invoke("upsertByExample", tb.getObject(), tbExample.getObject());
